...
@NullMarked
public class LoggingCompressProgressListener implements ProgressListener {

    private final Log log;

    public LoggingCompressProgressListener(final Log log) {
        this.log = log;
    }
    ...
    public void processedFile(final Path inputFile, final Duration duration) {
        log.info("Compressed: " + inputFile.toString() + " in: " + formatDuration(duration));
    }

    @Override
    public void finished(final Path inputPath, final Path outputFile, final Duration duration) {
        log.info("Finished compressing: " + inputPath.toString() + " to: " + outputFile.toString() + " in: " + formatDuration(duration));
    }
}
